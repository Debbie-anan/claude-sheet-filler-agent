...
| B | 产品 | 从截图中提取产品名称 |
| C | wati | **不要写入此列**：用户手动填 WATI 数字（如 19194125792），agent 既不写入也不能用 null 清空。**写入 range 必须避开 C 列** |
...
| J | 邀评渠道 | **固定**: "Share Experience EDM" (dropdown) |
| K | 评论链接 | 从截图 URL 栏提取（用 url 对象格式写入） |
| L | 评论截图 | **仅当用户本轮在聊天窗口发了截图才写入**：写入时用 embed-image 嵌入图片（先调用图片上传接口拿 fileToken，再写 `{"type":"embed-image","fileToken":"..."}`），无条件覆盖原值。**不能写成 type=url 的超链接**（表格会显示成可点击文件名而非缩略图）。如果用户**没**在窗口发截图，则跳过此列（保留原值，不写不清空） |
| M | 星级 | 文本格式 "X.0"（如 "5.0"、"4.0"），注意是字符串不是数字。**判断方法见下方"星级判断规则"** |
...
**重要**：A 列（状态）、C 列（WATI）、E 列（礼品选择）、L 列（评论截图）由用户手动管理，写入时这四列**绝不能出现在 range 里**——用 null 占位会清空已有值。所以拆成多段写：
...
第三段 F:K（**跳过 E 列礼品选择**）：
```bash
lark-cli sheets +write --spreadsheet-token TMJms0RiMhJTK7t0I3lcT44tn6g --sheet-id d352f1 --range "F{row}:K{row}" --as bot --profile bot2 --values '[[
  null,
  "赖丹",
  {日期序列号},
  {"type":"multipleValue","values":["站点选项"]},
  {"type":"multipleValue","values":["Share Experience EDM"]},
  {"type":"url","text":"评论链接","link":"评论链接"}
]]'
```

第四段 L（**仅当用户本轮发了新截图才执行**，否则跳过）：
用图片上传接口（如 `lark-cli sheets +write-image`）写入 embed-image 类型，无条件覆盖原值。**不要写成 url 超链接**——历史踩过坑，表格只会显示文件名超链接，不显示缩略图。
```bash
lark-cli sheets +write-image --spreadsheet-token TMJms0RiMhJTK7t0I3lcT44tn6g --sheet-id d352f1 --range "L{row}:L{row}" --as bot --profile bot2 --image-path "<截图本地路径>"
```

第五段 M:P（星级、Amazon profile、评论类型）：
```bash
lark-cli sheets +write --spreadsheet-token TMJms0RiMhJTK7t0I3lcT44tn6g --sheet-id d352f1 --range "M{row}:P{row}" --as bot --profile bot2 --values '[[
  "X.0",
  "profile name 或 null",
